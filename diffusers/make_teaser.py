...
if args.mode == 'jigsaw':
    ...
    # No cross-fade: the disk content is purely image1 rotated by the current
    # angle, and the outer ring is image1 throughout (image1 and image2 are
    # byte-identical outside the disk, so it doesn't matter which we use).
    # The rotation itself produces the correct image2 at t=1.
    def render_frame_at_t(t, caption=''):
